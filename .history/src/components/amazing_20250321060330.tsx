...
function CategoryCard({ image, title }: { image: string; title: string }) {
  ...
  return (
    <motion.div className="flex flex-col items-center" variants={itemVariants}>
      <div className="w-48 h-40 flex items-center justify-center shadow-md border border-white">
        <Image src={`/${image}`} alt={title} width={120} height={120} className="object-cover" />
      </div>
      <div className="bg-[#FFCA28] w-48 py-2 px-4 text-center w-full text-black font-medium">
